...
// parseTimeRange converts time range string to start/end times
...
func (s *sensorAnalyticsService) GetPaginatedData(timeRange string, page, pageSize int) (*models.SensorDataResponse, error) {
	...
	// Get temperature data with pagination
	var tempData []models.SensorTemperature
	var totalTemp int64

	offset := (page - 1) * pageSize

	err := s.db.Model(&models.SensorTemperature{}).
		Where("timestamp BETWEEN ? AND ?", start, end).
		Count(&totalTemp).Error
	if err != nil {
		return nil, err
	}
	...
	// Get corresponding humidity data
	var combinedData []models.CombinedSensorData
	for _, temp := range tempData {
		var humid models.SensorHumidity
		// Find humidity reading closest to temperature timestamp
		s.db.Where("timestamp <= ?", temp.Timestamp.Add(5*time.Second)).
			Order("timestamp DESC").
			First(&humid)
...
func (s *sensorAnalyticsService) GetHourlyData(timeRange string) ([]models.HourlyData, error) {
	...
	for hour := 0; hour < 24; hour++ {
		hourStart := time.Date(start.Year(), start.Month(), start.Day(), hour, 0, 0, 0, start.Location())
		hourEnd := hourStart.Add(1 * time.Hour)

		// Skip if hour is outside our range
		if hourEnd.Before(start) || hourStart.After(end) {
			continue
		}

		// Get temperature data for this hour
		var tempData []models.SensorTemperature
		s.db.Where("timestamp BETWEEN ? AND ?", hourStart, hourEnd).
			Find(&tempData)

		// Get humidity data for this hour
		var humidData []models.SensorHumidity
		s.db.Where("timestamp BETWEEN ? AND ?", hourStart, hourEnd).
			Find(&humidData)
